...
export async function generateFixture(tournamentId: string) {
  ...
  if (tournament.format !== "SINGLE_ELIM") {
    throw new Error("Por ahora el generador automático soporta eliminación simple; el resto de formatos vienen en la siguiente iteración.");
  }

  const bracket = generateSingleEliminationBracket(
    tournament.teams.map((t) => ({ id: t.id, name: t.name, seed: t.seed ?? 999 }))
  );
...
/** Carga resultado y, si corresponde, avanza al ganador a la siguiente ronda. */
